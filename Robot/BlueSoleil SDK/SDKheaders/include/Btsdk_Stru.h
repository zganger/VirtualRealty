...
typedef enum _BTSDK_GATT_SERVICE_TYPE {
	BTSDK_GATT_SERVICE_TYPE_PRIMARY,
	BTSDK_GATT_SERVICE_TYPE_SECONDARY  
} BTSDK_GATT_SERVICE_TYPE;

typedef enum _BTSDK_GATT_READ_PERMISSION {
	BTSDK_GATT_NOT_READABLE,
	BTSDK_GATT_READABLE_WITHOUT_SECURITY,
	BTSDK_GATT_READABLE_WITH_AUTHENTICATION,
	BTSDK_GATT_READABLE_WITH_ENCRYPTION
} BTSDK_GATT_READ_PERMISSION;

typedef enum _BTSDK_GATT_WRITE_PERMISSION {
	BTSDK_GATT_NOT_WRITABLE,
	BTSDK_GATT_WRITABLE_WITHOUT_SECURITY,
	BTSDK_GATT_WRITABLE_WITH_AUTHENTICATION,
	BTSDK_GATT_WRITABLE_WITH_ENCRYPTION
} BTSDK_GATT_WRITE_PERMISSION;

/* Permissions of Characteristic Descriptor */
typedef struct _BtsdkGATTDescriptorPermissionStru {
	BTSDK_GATT_READ_PERMISSION     ReadPermission;  /* Read Permission */
	BTSDK_GATT_WRITE_PERMISSION    WritePermission; /* Write Permission */
} BtsdkGATTDescriptorPermissionStru, *PBtsdkGATTDescriptorPermissionStru;

typedef enum _BTSDK_GATT_WRITE_TYPE {
    BTSDK_GATT_WRITE_WITH_RESPONSE,         /* Write Request */
    BTSDK_GATT_WRITE_WITHOUT_RESPONSE,      /* Write Command */
    BTSDK_GATT_WRITE_PREPARE,               /* Prepare Write */
} BTSDK_GATT_WRITE_TYPE;
...
/* Parameter of Characteristic Value Read operation */
typedef struct _BtsdkGATTCharacteristicReadRequestStru {
	BTDEVHDL            hDevice;            /* Handle to the remote device that has requested the read operation */
	BtsdkGATTUUIDStru   CharacteristicUuid; /* Characteristic UUID */
	BTUINT16            ServiceHandle;      /* Service Definition Attribute Handle */
	BTUINT16		    ValueHandle;        /* Characteristic Value Attribute Handle */
	BTUINT32            Offset;             /* Value offset for partial read operation */
} BtsdkGATTCharacteristicReadRequestStru, *PBtsdkGATTCharacteristicReadRequestStru;

/* Parameter of Characteristic Value Write operation */
typedef struct _BtsdkGATTCharacteristicWriteRequestStru {
	BTSDK_GATT_WRITE_TYPE  TypeWrite;       /* Type of write operation */
	BTDEVHDL            hDevice;            /* Handle to the remote device that has requested the write operation */
	BtsdkGATTUUIDStru   CharacteristicUuid; /* Characteristic UUID */
	BTUINT16            ServiceHandle;      /* Service Definition Attribute Handle */
	BTUINT16		    ValueHandle;        /* Characteristic Value Attribute Handle */
	BTUINT32            Offset;             /* Value offset for partial write operation */
	BTUINT32            DataSize;           /* Size, in bytes, of the value data to be written */
	BTUINT8             Data[1];            /* The value data to be written */
} BtsdkGATTCharacteristicWriteRequestStru, *PBtsdkGATTCharacteristicWriteRequestStru;

/* Parameter of Descriptor Value Read operation */
typedef struct _BtsdkGATTDescriptorReadRequestStru {
	BTDEVHDL            hDevice;            /* Handle to the remote device that has requested the read operation */
	BtsdkGATTUUIDStru   DescriptorUuid;     /* Descriptor UUID */
	BTUINT16            ServiceHandle;      /* Service Definition Attribute Handle */
	BTUINT16		    ValueHandle;        /* Characteristic Value Attribute Handle */
	BTUINT16		    DescriptorHandle;   /* Descriptor Declaration Attribute Handle */
	BTUINT32            Offset;             /* Value offset for partial read operation */
} BtsdkGATTDescriptorReadRequestStru, *PBtsdkGATTDescriptorReadRequestStru;

/* Parameter of Descriptor Value Write operation */
typedef struct _BtsdkGATTDescriptorWriteRequestStru {
	BTSDK_GATT_WRITE_TYPE  TypeWrite;       /* Type of write operation */
	BTDEVHDL            hDevice;            /* Handle to the remote device that has requested the write operation */
	BtsdkGATTUUIDStru   CharacteristicUuid; /* Characteristic UUID */
	BTUINT16            ServiceHandle;      /* Service Definition Attribute Handle */
	...
	BTUINT32            DataSize;           /* Size, in bytes, of the value data to be written */
	BTUINT8             Data[1];            /* The value data to be written */
} BtsdkGATTDescriptorWriteRequestStru, *PBtsdkGATTDescriptorWriteRequestStru;

/* Parameter of Execute Write operation */
typedef struct _BtsdkGATTExecuteWriteRequestStru {
	BTDEVHDL            hDevice;            /* Handle to the remote device that has requested the write operation */
	BTBOOL              Execute;            /* Whether the pending writes should be executed (BTSDK_TRUE) or cancelled (BTSDK_FALSE) */
} BtsdkGATTExecuteWriteRequestStru, *PBtsdkGATTExecuteWriteRequestStru;
