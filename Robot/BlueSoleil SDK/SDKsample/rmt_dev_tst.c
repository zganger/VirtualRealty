...
BTUINT8 AppObexAuthenticationReqInd(BTCONNHDL conn_hdl, PBtSdkObexAuthInfoStru auth_info)
{
	...
	if (i < num)
	{
		if (!auth_info->pwd_only)
		{
			...
			AppWaitForInput(auth_info->user_id, BTSDK_MAX_USERID_LEN+1);
			
		}
		printf("Please input the password for remote OBEX service:\r\n");
		//You can get the pwd from other GUI.
		AppWaitForInput(auth_info->pwd, BTSDK_MAX_PWD_LEN+1);
...
void AppVendorEventInd(BTUINT8 ev_code, BTUINT8 ev_param_size, BTUINT8 *ev_param)
{
	printf("Vendor event indication the parameter length is:%x\n", ev_param_size);
}
>>>>>>> refs/remotes/origin/master
...
=======
	...
	cb.type = BTSDK_VENDOR_EVENT_IND;
	cb.func = (void*)AppVendorEventInd;
	Btsdk_RegisterCallback4ThirdParty(&cb);
>>>>>>> refs/remotes/origin/master
}
